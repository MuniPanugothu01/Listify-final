/**
 * Production-Grade Centralized Logger — AWS CloudWatch
 *
 * Log Groups (auto-created in CloudWatch):
 *   /listify/production/application   — general app info, warnings, debug
 *   /listify/production/errors        — all error-level entries (for alarms)
 *   /listify/production/products      — product lifecycle: posted/updated/deleted
 *   /listify/production/users         — auth events: login, register, logout, password
 *   /listify/production/security      — CSRF blocks, rate limits, suspicious activity
 *
 * Security:
 *   • PII (emails, passwords, tokens, OTPs) is hashed/redacted before shipping
 *   • Logs encrypted at rest via CloudWatch default SSE (AES-256)
 *   • IAM policy should be scoped to logs:* only
 *   • Retention automatically set to 90 days (configurable via LOG_RETENTION_DAYS)
 *
 * In development (NODE_ENV !== 'production') logs print to console ONLY.
 * Local file transports are REMOVED — all production logs go to CloudWatch.
 */
...
// ══════════════════════════════════════════════════════════════════
// PII SANITIZER — redact sensitive data before it leaves the server
// ══════════════════════════════════════════════════════════════════
...
// ── Winston Format: sanitize all meta before output ───────────────
...
// ══════════════════════════════════════════════════════════════════
// HELPER: Request Log (for Morgan / middleware)
// ══════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════
// GRACEFUL SHUTDOWN — flush pending CloudWatch batches
// ══════════════════════════════════════════════════════════════════
const flushLogs = () => {
