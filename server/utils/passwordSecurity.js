...
const checkPasswordHistory = async (userId, newPassword) => {
  try {
    ...
    if (user.password) {
      const isSameAsCurrent = await require('bcryptjs').compare(newPassword, user.password);
      if (isSameAsCurrent) {
        return {
          reused: true,
    ...
    for (let i = 0; i < user.passwordHistory.length; i++) {
      ...
      const isMatch = await require('bcryptjs').compare(newPassword, historyItem.password);
      if (isMatch) {
        return {
          reused: true,
