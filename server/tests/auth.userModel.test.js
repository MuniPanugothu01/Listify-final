...
describe('👤 USER MODEL TESTS', () => {
  ...
  describe('1. Password Comparison (bcrypt)', () => {
    test('TC-U01: bcrypt.compare should return true for matching passwords', async () => {
    ...
    test('TC-U02: bcrypt.compare should return false for non-matching passwords', async () => {
      const plain = 'SecureP@ss1!';
      const salt = await bcrypt.genSalt(12);
      const hashed = await bcrypt.hash(plain, salt);

      const result = await bcrypt.compare('WrongP@ss1!', hashed);
      expect(result).toBe(false);
    });

    test('TC-U03: Should use 12 rounds of bcrypt (salt rounds)', async () => {
      const plain = 'TestPassword1!';
      const salt = await bcrypt.genSalt(12);
      const hashed = await bcrypt.hash(plain, salt);

      // bcrypt hash format: $2a$12$...  (12 = rounds)
