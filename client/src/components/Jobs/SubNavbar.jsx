            ...
            {subNavItems.map((item, index) => (
              <div key={index} className="dropdown-container relative">
                <button
                  ...
                  className={`flex items-center justify-center px-4 py-2 rounded-full text-sm transition-all whitespace-nowrap min-w-max cursor-pointer ${
                    item.name === ""
                      ? "text-[#27bb97] hover:text-[#1FA987] px-3"
                      : "text-gray-700 hover:text-[#27bb97]"
                  } ${
                    activeDropdown === item.name ? "text-[#27bb97] font-semibold" : ""
                  }`}
                ...
                {activeDropdown === item.name && hasDropdown(item.name) && (
                  <div 
                    className="absolute top-full left-0 z-50 mt-1 min-w-[220px] bg-white rounded-lg shadow-xl border border-gray-200 overflow-hidden"
                  >
                    {/* Dropdown Header */}
                    <div className="px-4 py-3 bg-teal-50 border-b border-teal-100">
                      <h3 className="text-sm font-semibold text-teal-700">{item.name}</h3>
                    </div>
