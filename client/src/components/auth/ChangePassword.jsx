...
const ChangePassword = ({ onSuccess }) => {
  ...
  useEffect(() => {
    ...
    setPasswordChecks((prev) => ({
      ...
      uppercase: passwordRequirements.requireUppercase
        ? /[A-Z]/.test(password)
        : true,
      lowercase: passwordRequirements.requireLowercase
        ? /[a-z]/.test(password)
        : true,
      number: passwordRequirements.requireNumbers
        ? /[0-9]/.test(password)
        : true,
      special: passwordRequirements.requireSpecialChars
        ? new RegExp(
            `[${passwordRequirements.specialChars.replace(/[-\/\\^$*+?.()|[\]{}]/g, "\\$&")}]`,
          ).test(password)
        : true,
    }));
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {passwordChecks.breachChecked && passwordChecks.isBreached && (
              <div className="mt-2 p-2 bg-red-50 border border-red-200 rounded-lg flex items-start gap-2">
                ...
                <p className="text-xs text-red-700">
                  This password has been exposed in a data breach. Please choose
      ...
      <div className="bg-blue-50 p-3 rounded-lg border border-blue-200 flex items-start gap-2">
        ...
        <p className="text-xs text-blue-700">
          Your new password will be checked against known data breaches and
          added to your password history. You cannot reuse any of your last 5
          passwords.
        </p>
      </div>
