...
import { updateProfile, uploadProfileImage, setImageUploadProgress } from "../../redux/slices/profileSlice";
...
const PersonalDetailsSection = ({
  ...
  const { imageUploadProgress, imageUploading: reduxImageUploading } = useSelector((state) => state.profile);
  
  const [isUploading, setIsUploading] = useState(false);
  const [uploadProgress, setUploadProgress] = useState(0);

  // Sync with Redux upload state
  useEffect(() => {
    if (reduxImageUploading || externalIsUploading) {
      setIsUploading(true);
  ...
  }, [reduxImageUploading, externalIsUploading, imageUploadProgress]);
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center gap-6 md:gap-8">
            <div className="relative">
              <div className="relative">
                <div className={`relative ${isEditing ? "cursor-pointer" : ""}`}>
                  ...
                  <div className="relative">
                    <img
                      src={profilePicPreview || "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=100&h=100&fit=crop"}
                      alt="Profile"
                      className={`w-24 h-24 md:w-28 md:h-28 rounded-full object-cover border-4 shadow-lg transition-all duration-300 ${
                        isUploading 
                    ...
                    {isUploading && (
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="text-lg font-bold text-emerald-600">{uploadProgress}%</div>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {/* Verified Badge */}
                {!isEditing && !isUploading && user?.isVerified && (
            ...
            <div className="flex-1">
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-3">
                <h3 className="text-xl md:text-2xl font-bold text-gray-900">{editData?.name || "User"}</h3>
              ...
              <p className="text-gray-600 mb-6">{editData?.email || "No email provided"}</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                ...
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                  ...
                  <div>
                    ...
                    <div className="text-xs md:text-sm text-gray-600">{user?.rating || "4.8"}/5</div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                  ...
                  <div>
                    ...
                    <div className="text-xs md:text-sm text-gray-600">{user?.propertiesCount || "0"} Active</div>
                  </div>
                </div>
              </div>
            </div>
