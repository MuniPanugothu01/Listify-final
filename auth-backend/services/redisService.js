...
class RedisService {
  ...
  static async verifyOTP(email, otp) {
    try {
      ...
      console.log(`[Redis] Verifying OTP for ${email}: received=${otp} (type: ${typeof otp}), stored=${storedOTP} (type: ${typeof storedOTP})`);
  ...
  static async checkEmailBlocked(email) {
    try {
      ...
      console.log(`[Redis] Check email blocked ${email}: ${exists ? 'Yes' : 'No'}`);
      return exists === 1;
    } catch (error) {
